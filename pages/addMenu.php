...
if (isset($_GET['menuId'])) {
    $menuId = $_GET['menuId'];
    $menuDetails =$obj->fetchMenuDetailsFromDatabase($menuId); 
}
    ...
    <div class="w-screen h-screen">
        <div class="flex flex-col md:flex-row lg:flex-row w-screen h-screen">
            ...
            ?>
                    <div id="showMenu">
                        <div class="flex gap-5 justify-center items-center" id="showMenu">
                            ...
                            <a href="#" id="addItem"
                                class="bg-blue-500 m-2 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded">Add
                                item</a>
                        </div>
                    ...
                    if(count($allFoodMenu) > 0 ){
                      for ( $i = 0; count($allFoodMenu) > $i; $i++ ) {  ?>
                                    <tr>
                                        <td class="  p-2 border"><?=$allFoodMenu[$i]['foodName']?></td>
                                        <td class="  p-2 border"><?=$allFoodMenu[$i]['foodPrice']?></td>
                                        <td class="  p-2 border">
                                            <img src="../resourses/foodMenuImages/<?=$allFoodMenu[$i]['menuImage']?>"
                                                height="100px" width="100px" alt="<?=$allFoodMenu[$i]['foodName']?>" />
                                        </td>
                                        <td class="  p-2 border flex gap-3">
                                            <a href="addMenu.php?restaurantId=<?=$restaurantId?>&menuId=<?=$allFoodMenu[$i]['id']?>"
                                                class="bg-blue-500 editMenuButton m-2 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded">
                                                Edit
                                            </a>

                                            <a href="../controler/menuDeleteForRestaurant.php?restaurantId=<?=$restaurantId?>&menuId=<?=$allFoodMenu[$i]['id']?>"
            ...
            ?>
                    ...
                    <form action="../controler/menuAddForRestaurant.php" id="addMenuform" method="POST"
                ...
                if (isset($_GET['menuId'])){
              ?>
                    <form action="../controler/menuUpdateForRestaurant.php" id="editMenu" method="POST"
                        class="space-y-4  w-2/4" enctype="multipart/form-data">
                        <h1 class="text-2xl text-slate-900 text-center">Edit Menu</h1>
                        <div class="flex gap-5 justify-center items-center">
                            <label for="foodName" class="block text-sm font-bold text-black">Food Name</label>
                            <input type="text" name="foodName" id="foodName" value="<?=$menuDetails['foodName']?>"
                                class="mt-1 p-2 rounded border" placeholder="Username" />
                            <input type="number" name="menuId" id="menuId" value=<?=$menuDetails['id']?>
                                class="mt-1 p-2 hidden rounded border" placeholder="Username" />
                        </div>
                        <div class="flex gap-8 justify-center items-center">
                            <label for="foodPrice" class="block text-sm font-bold text-black">Food Price</label>
                            <input type="Number" name="foodPrice" id="foodPrice" value=<?=$menuDetails['foodPrice']?>
                                class="mt-1 p-2 rounded border" placeholder="Food Price" />
                        </div>
